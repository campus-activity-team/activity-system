...
- `GET /api/organizer/activities/{activityId}/announcements`：活动发起者或管理员查看公告历史
- `POST /api/organizer/activities/{activityId}/announcements`：活动发起者或管理员发布公告，标题最多 120 个字符、内容最多 2000 个字符

报名成功、活动审核/发布/下架、活动取消、活动公告、活动反馈以及发起者申请审批结果会生成站内通知。反馈提交后会通知活动发起者和所有有效管理员，通知内容不包含评价者身份。公告仅允许在 `PUBLISHED` 或 `ONGOING` 状态发布，发送给发布时所有有效报名者；已取消或已结束活动保留历史公告但不能发布新公告。后台每分钟检查一次未来 24 小时内开始的已发布活动，为保持有效报名的用户生成一次开场提醒；提醒使用唯一键去重，不会因轮询重复发送。活动审核、发布、下架、取消、手动开启/结束、手动补签/撤销、公告发布、反馈提交、身份审批和角色变更等关键动作会写入管理员可查询的操作日志。
