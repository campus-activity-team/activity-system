...
import { approveActivity, cancelActivity, endActivity, getAdminActivities, publishActivity, rejectActivity, startActivity, unpublishActivity } from '../../api/activity'
...
async function end(id: number) {
  try {
    await ElMessageBox.confirm('确定立即结束活动？结束后将停止签到并进入反馈阶段。', '手动结束活动', { type: 'warning' })
...
async function cancel(row: Activity) {
  try {
    const result = await ElMessageBox.prompt('请输入取消原因，活动发起者和所有报名者都会收到通知。', `取消“${row.title}”`, {
      type: 'warning',
      inputPattern: /\S+/,
      inputErrorMessage: '取消原因不能为空',
    })
    await cancelActivity(row.id, result.value.trim())
    ElMessage.success('活动已取消并发送通知')
    await loadReviews()
  } catch (error: any) {
...
<template>
  <div class="page-card">
    ...
    <el-table v-else v-loading="loading" :data="activities" stripe>
      ...
      <el-table-column label="操作" min-width="440"><template #default="{ row }"><el-button v-if="row.status === 'PENDING_REVIEW'" type="success" size="small" @click="approve(row.id)">通过</el-button><el-button v-if="row.status === 'PENDING_REVIEW'" type="danger" size="small" @click="reject(row.id)">驳回</el-button><el-button v-if="row.status === 'APPROVED'" type="primary" size="small" @click="publish(row.id)">发布</el-button><el-button v-if="row.status === 'PUBLISHED'" type="success" size="small" @click="start(row.id)">手动开启</el-button><el-button v-if="row.status === 'ONGOING'" type="danger" size="small" @click="end(row.id)">立即结束</el-button><el-button v-if="row.status === 'PUBLISHED' || row.status === 'ONGOING'" type="warning" size="small" @click="unpublish(row.id)">下架</el-button><el-button v-if="row.status === 'APPROVED' || row.status === 'PUBLISHED' || row.status === 'ONGOING'" type="danger" size="small" plain @click="cancel(row)">取消活动</el-button></template></el-table-column>
    </el-table>
  </div>
</template>
