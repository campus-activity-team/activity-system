...
这是一个前后端分离的高校/机构活动报名与签到系统。目前已完成 **Phase 1-5**：项目基础、用户认证、活动生命周期与审核、活动报名与签到、活动反馈、通知与运营管理。
...
后端容器默认使用 `Asia/Shanghai` 时区；其他地区部署可在 `.env` 设置 `APP_TIME_ZONE`。
...
- 活动发起者可撤回审核；发起者或管理员可填写原因取消待发布、已发布或进行中的活动，系统通知全部有效报名者
...
- 站内通知：报名成功、活动审核、发起者申请结果实时通知，已报名用户在活动开始前 24 小时内收到一次提醒
- 管理员操作日志：按操作类型、对象类型和操作人查询活动审核、权限、签到及活动控制记录
...
同时会为 `organizer` 创建 7 个覆盖不同状态的测试活动，为进行中的签到演示活动创建三个测试报名记录，并为往期演示活动创建两名已签到参与者及一份匿名反馈。`student3` 默认已报名但未签到，可用于测试手动补签；`student` 可直接测试提交反馈，`student2` 可查看或修改预置反馈，`organizer` 可查看统计看板。登录后可从顶部“通知”入口查看站内消息，管理员可从“操作日志”入口查询关键操作。
